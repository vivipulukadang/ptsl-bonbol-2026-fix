...
// Aturan Vivi: 1 baris per petugas (tidak digabung), 1 halaman per hari ST,
// kolom Hari & Tanggal diisi sesuai tanggal ST per halaman. Pelaksana sama di
// tiap halaman. Sel Pejabat yang Mengesahkan (Kades) dibiarkan apa adanya.
function dayInfo(iso) {
  const d = parseISO(iso);
  if (!d) return { hari: '', tanggal: '' };
  return { hari: HARI[d.getDay()], tanggal: `${d.getDate()} ${BULAN[d.getMonth()]} ${d.getFullYear()}` };
}
// Ganti seluruh isi sel (setelah tcPr) dengan satu run terlihat (Bookman 11pt, center).
function setCellText(tc, value) {
  const m = tc.match(/^(<w:tc>(?:\s*<w:tcPr>[\s\S]*?<\/w:tcPr>)?)/);
  const head = m ? m[1] : '<w:tc>';
  const para = `<w:p><w:pPr><w:jc w:val="center"/></w:pPr><w:r>` +
    `<w:rPr><w:rFonts w:ascii="Bookman Old Style" w:hAnsi="Bookman Old Style"/><w:sz w:val="22"/><w:szCs w:val="22"/></w:rPr>` +
    `<w:t xml:space="preserve">${xmlEscape(value)}</w:t></w:r></w:p>`;
  return head + para + '</w:tc>';
}
function buildFormKehadiran(xml, petugas, tglList) {
  // tabel form = tabel yang mengandung TOKEN (Pelaksana 'xxx')
  const tables = xml.match(/<w:tbl>[\s\S]*?<\/w:tbl>/g) || [];
  const formTbl = tables.find((t) => t.includes(TOKEN));
  if (!formTbl) return xml; // tidak ada form Pelaksana (mis. bukan Puldadis) -> lewati

  const rows = formTbl.match(/<w:tr\b[\s\S]*?<\/w:tr>/g) || [];
  const dataRow = rows.find((r) => r.includes(TOKEN));
  if (!dataRow) return xml;

  const list = (petugas && petugas.length) ? petugas : [{}];
  const days = (tglList && tglList.length ? tglList : [null]).map(dayInfo);

  // bangun satu baris per petugas untuk satu hari tertentu
  const buildRows = (day) => list.map((p, idx) => {
    const nama = p.Nama_Cache || p.Nama || '';
    let ci = -1;
    return dataRow.replace(/(<w:tc>[\s\S]*?<\/w:tc>)/g, (tc) => {
      ci++;
      if (tc.includes(TOKEN)) return tc.replace(TOKEN, xmlEscape(nama));      // Pelaksana
      if (tc.includes('Hari_Pelaksanaan_PL')) return setCellText(tc, day.hari);
      if (tc.includes('Tanggal_Pelaksanaan_PL')) return setCellText(tc, day.tanggal);
      if (ci === 0) return setCellText(tc, String(idx + 1));                  // No
      return tc; // sel Pejabat Mengesahkan dll dibiarkan
    });
  }).join('');

  // satu tabel form per hari (header + No.baris petugas)
  const pageBreak = '<w:p><w:r><w:br w:type="page"/></w:r></w:p>';
  const perDayTables = days.map((day) => formTbl.replace(dataRow, buildRows(day)));
  const replacement = perDayTables.join(pageBreak);

  return xml.replace(formTbl, replacement);
}
...
export async function generateAndDownloadST(stResult, skData, panitiaList) {
  ...
  // Kapitalisasi: badan surat pakai Title Case (Bongohulawa); judul Form pakai UPPER.
  const desaT = toTitleCase(desa);
  const desaU = String(desa).toUpperCase();
  const kecU = String(kec).toUpperCase();
  ...
  if (jenis === 'PULDADIS' || jenis === 'PULDADIS_DESA') {
    ...
    const mapper = (jenis === 'PULDADIS')
      ...
      : (p) => [p.Nama_Cache || p.Nama || '', desaT];
    xml = fillPetugasTable(xml, petugas, mapper);

    // Pelaksana Form Bukti Kehadiran ditangani terpisah (1 baris/petugas, 1 halaman/hari)
    if (jenis === 'PULDADIS') {
      // 12 token (tabel petugas sudah terisi); token Pelaksana di form disisakan
      xml = replaceInOrder(xml, [
        stResult.No_ST || '', skNomor, skTanggal, skTentang,
        desaT, kec, rangeDayMonth(tglList), String(jumlahHari), terbilang(jumlahHari),
        fmtDayMonth(tglST), desaU, kecU,
      ]);
      xml = buildFormKehadiran(xml, petugas, tglList);
    } else {
      xml = replaceInOrder(xml, [
        stResult.No_ST || '', skNomor, skTanggal, skTentang,
        desaT, kec, rangeDayMonth(tglList), String(jumlahHari), terbilang(jumlahHari),
    ...
    xml = replaceInOrder(xml, [
      ...
      desaU,                 // 16 form DESA
      kecU,                  // 17 form KECAMATAN
      kades,                 // 18 form baris 7 Kades nama
      desaT,                 // 19 form "Kepala Desa xxx"
    ]);
  }
