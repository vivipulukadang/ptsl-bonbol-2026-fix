...
// W-Layout: bagi N petugas jadi baris-baris. Sampai 4 -> satu baris.
// >4 -> maksimum 4 per baris, baris atas >= baris bawah.
function wlayout(n) {
  if (n <= 0) return [];
  if (n <= 4) return [n];
...
function buildDesaSheet(desa, kecamatan, rows, petugas, statusTanah) {
  ...
  // ---- area tanda tangan (W-Layout, heading PER petugas) ----
  const ttdMerges = [];
  let curRow = dr + 2;
  const layout = wlayout(petugas.length);
  let pIdx = 0;
  layout.forEach((cnt) => {
    const blockW = Math.min(6, Math.floor(NCOL / cnt)); // <=4/baris muat di 18 kolom
    const totalW = cnt * blockW;
    const startC = Math.max(0, Math.floor((NCOL - totalW) / 2));
    const rHead1 = curRow;       // "Petugas Pengumpul Yuridis"
    const rHead2 = curRow + 1;   // "PTSL 2026"
    const rName  = curRow + 5;   // nama (ruang tanda tangan 3 baris di atas nama)
    for (let k = 0; k < cnt; k++) {
      const c0 = startC + k * blockW;
      const c1 = c0 + blockW - 1;
      const nm = petugas[pIdx++] || '';
      setCell(ws, rHead1, c0, 'Petugas Pengumpul Yuridis', st({ border: false, sz: 9 }));
      setCell(ws, rHead2, c0, 'PTSL 2026', st({ border: false, sz: 9 }));
      setCell(ws, rName, c0, nm, st({ bold: true, border: false, sz: 10 }));
      ttdMerges.push({ s: { r: rHead1, c: c0 }, e: { r: rHead1, c: c1 } });
      ttdMerges.push({ s: { r: rHead2, c: c0 }, e: { r: rHead2, c: c1 } });
      ttdMerges.push({ s: { r: rName, c: c0 }, e: { r: rName, c: c1 } });
    }
    curRow = rName + 2;
  });
...
export async function generateLampiranIB({
  ...
  desaList = [],
  petugas = [],
  petugasByDesa = null,   // { 'NAMA DESA (UPPER)': ['Nama1','Nama2',...] }
  ...
  const hasPerDesa = petugasByDesa && Object.keys(petugasByDesa).length > 0;
  if (!hasPerDesa && petugasList.length === 0) {
    throw new Error('Pilih minimal satu petugas.');
  }
  ...
  for (const key of desaKeys) {
    ...
    const g = byDesa[key];
    const petForDesa = hasPerDesa
      ? ((petugasByDesa[key] || petugasByDesa[g.desa] || []).map((s) => String(s).trim()).filter(Boolean))
      : petugasList;
    const ws = buildDesaSheet(g.desa, g.kecamatan, g.rows, petForDesa, statusTanah);
