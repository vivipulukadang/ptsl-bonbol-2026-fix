...
import {
  ...
  getDesaByTahapan, getPanitiaPerSK, getKalendar, getMasterDesa,
...
// ─── Konstanta ────────────────────────────────────────────────
const toTitleCase = (s) => String(s == null ? '' : s).toLowerCase()
...
// Puldadis/PuldadisDesa: tanggal ST sebelum tanggal mulai (iso < tglMulai)
// PL/Sidang: setelah tanggal mulai, sebelum tanggal pengumuman (tglMulai < iso < tglPengumuman)
...
function KalenderST({ kalenderData, tglMulai, tglPengumuman, jenisValue, jumlahHari, selected, onChange }) {
  ...
  const availableDates = useMemo(() => {
    ...
    const base = mk(tglMulai);
    const rangeStart = new Date(base); rangeStart.setDate(rangeStart.getDate() - 90);
    const rangeEnd   = tglPengumuman ? mk(tglPengumuman) : new Date(base);
    ...
    <div className="border border-gray-200 rounded-lg p-3 bg-white">
      ...
      {jenisValue === 'PULDADIS' || jenisValue === 'PULDADIS_DESA' ? (
        <p className="text-xs text-blue-600 mt-1.5 bg-blue-50 rounded px-2 py-1">
          Tanggal ST harus sebelum Tanggal Mulai ({tglMulai})
        </p>
      ) : (
        <p className="text-xs text-blue-600 mt-1.5 bg-blue-50 rounded px-2 py-1">
...
export default function BuatST() {
  ...
  const masterDesa = useFetch(getMasterDesa, {});
  ...
  const [f, setF] = useState({
    ...
    tgl_st_list:    [],
    petugas_ids:    [],
    bulk_indices:   [], // bulk yang dipilih (bisa lebih dari satu = join)
    nama_kades:     '', // auto dari master desa, bisa diedit
  });
  ...
  // Saat BULK dipilih (boleh lebih dari satu = join) → auto-fill dari gabungan bulk.
  // Tanggal mulai = paling awal dari bulk terpilih; tanggal pengumuman = paling awal juga.
  useEffect(() => {
    if (!f.bulk_indices.length || !evidenceParsed) {
      return;
    }
    const bulks = f.bulk_indices.map(i => evidenceParsed.bulkGroups[i]).filter(Boolean);
    if (!bulks.length) return;
    const mulai = bulks.map(b => b.tglMulai).filter(Boolean).sort();
    const pengumuman = bulks.map(b => b.tglPengumuman).filter(Boolean).sort();
    setF(p => ({
      ...p,
      id_desa: bulks[0].desa,
      tgl_mulai: mulai[0] || '',
      tgl_pengumuman: pengumuman[0] || '',
      tgl_st_list: [],
    }));
  }, [f.bulk_indices, evidenceParsed]);
  ...
  // Bulk options — tiap bulk = 1 baris pilihan (bisa di-join). Diberi index asli.
  ...
  // Jumlah bidang = total dari semua bulk terpilih
  const jumlahBidang = useMemo(() => {
    if (!f.bulk_indices.length || !evidenceParsed) return 0;
    return f.bulk_indices.reduce((sum, i) => {
      const b = evidenceParsed.bulkGroups[i];
      return sum + (b ? b.bidangList.length : 0);
    }, 0);
  }, [f.bulk_indices, evidenceParsed]);

  // Kades + jabatan penandatangan dari master desa (02_MASTER_DESA_KADES)
  const desaInfo = useMemo(() => {
    if (!f.id_desa || !masterDesa.data) return null;
    const target = f.id_desa.trim().toLowerCase();
    return (masterDesa.data || []).find(d =>
      String(d.Nama_Desa || d.nama_desa || '').trim().toLowerCase() === target
    ) || null;
  }, [f.id_desa, masterDesa.data]);
  const namaKades = desaInfo ? (desaInfo.Nama_Kades_Lurah || desaInfo.nama_kades_lurah || '') : '';
  const jabatanKades = desaInfo ? (desaInfo.Jabatan_Penandatangan || 'Kepala Desa') : 'Kepala Desa';

  // Auto-isi nama kades saat desa berubah (boleh ditimpa manual)
  useEffect(() => {
    setF(p => ({ ...p, nama_kades: namaKades }));
  }, [namaKades]);
  ...
  const handleStep1 = async () => {
    ...
    if (!f.bulk_indices.length && !f.id_desa) { setError('Pilih minimal satu bulk dari Evidence.'); return; }
  ...
  const handleStep2 = async () => {
    ...
    try {
      ...
      setStResult({
        ...
        namaDesa: f.id_desa,
        kecamatan: (evidenceParsed?.bulkGroups?.[f.bulk_indices[0]]?.kecamatan)
          || evidenceParsed?.desaStats?.[f.id_desa.toUpperCase()]?.kecamatan || '',
        jumlahBidang,
        namaKades: f.nama_kades || namaKades,
        jabatanKades: jabatanKades,
  ...
  const resetForm = () => {
    ...
    setF({ jenis:'PULDADIS', id_tahapan:'', id_sk_dasar:'', id_desa:'', jumlah_hari:1,
      tgl_mulai:'', tgl_pengumuman:'', tgl_st_list:[], petugas_ids:[], bulk_indices: [] });
        ...
        <div className="space-y-5">
          ...
          <UploadEvidencePanel
            onParsed={(result) => {
              ...
              setF(p => ({ ...p, bulk_indices: [], id_desa: '', tgl_mulai: '', tgl_pengumuman: '', tgl_st_list: [] }));
            }}
          ...
          <div className="card">
            ...
            {!evidenceParsed ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="label">Bulk * (boleh pilih lebih dari satu untuk join)</label>
                  <div className="border border-gray-200 rounded-lg max-h-44 overflow-y-auto divide-y">
                    {bulkOptions.length === 0 && (
                      <p className="text-xs text-gray-400 px-3 py-2">Upload Evidence dulu.</p>
                    )}
                    {bulkOptions.map(o => {
                      const checked = f.bulk_indices.includes(o.value);
                      return (
                        <label key={o.value}
                          className={`flex items-center gap-2 px-3 py-2 text-xs cursor-pointer hover:bg-gray-50 ${checked ? 'bg-navy/5' : ''}`}>
                          <input type="checkbox" checked={checked}
                            onChange={() => setF(p => ({
                              ...p,
                              bulk_indices: checked
                                ? p.bulk_indices.filter(x => x !== o.value)
                                : [...p.bulk_indices, o.value],
                              tgl_st_list: [],
                            }))} />
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {f.bulk_indices.length} bulk dipilih{f.bulk_indices.length > 1 ? ' — di-join jadi satu ST' : ''}
                  </p>
                </div>
                <div>
            ...
            {evidenceParsed && f.id_desa && (
              <div className="mt-4">
                <label className="label">Nama Kepala Desa / Lurah (auto dari master)</label>
                <input type="text" className="input-field" value={f.nama_kades}
                  onChange={e => set('nama_kades', e.target.value)}
                  placeholder={masterDesa.loading ? 'Memuat master desa...' : 'Tidak ada di master, isi manual'} />
                <p className="text-xs text-gray-400 mt-1">
                  {jabatanKades} {f.id_desa ? `\u2014 Desa ${toTitleCase(f.id_desa)}` : ''}. Dipakai di ST Pemeriksaan Lapang &amp; Sidang Panitia.
                </p>
              </div>
            )}
          </div>
